...
import 'package:meca_note_mobile/widgets/loding_widget.dart';
                    ...
                    child: Column(
                      children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            if(_loading){
                              NotificationHelper.success(context, "Tantative de connexion en cours.");
                            }else{
                              setState(() {
                                _loading = true;
                              });
                              ...
                              if (response['data']['status'] == 'OK') {
                                ...
                                var res = await PushNotificationService.updateDeviceToken(await PushNotificationService.getDeviceToken());
                                print(res);
                                NotificationHelper.success(context, response['data']['message']);

                                setState(() {
                                  _loading = false;
                                });
                                var username = await ApiConfig.getUsername();
                                if(username == 'MECANO'){
                                  Navigator.push<void>(
                                    context,
                                ...
                                NotificationHelper.error(context, response['data']['message']);
                              }
                            }
                          },
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                color: _loading ? ColorWidget.blue?.withOpacity(0.5): ColorWidget.blue),
                            child: const Center(
                              child: Text(
                                "se connecter",
                                style: TextStyle(
                        ...
                        const Text("ou se connecter avec", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600),),
                         const SizedBox(height: 20,),
                         Row(
                           mainAxisAlignment: MainAxisAlignment.center,
                          children: [
